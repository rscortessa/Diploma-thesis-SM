...
for ii in range(len(L)):
    ...
    File=pd.read_csv("./graph4/DP_L"+str(L[ii])+"T"+str(t[ii])+"P("+str(pp)+"-"+str(pp+dp)+")S"+str(sites[ii])+".txtaux",delim_whitespace=True,header=None)
    print(A[ii,:])
    A[:,ii]=File[0]
    B[:,ii]=File[1]
...
for i in range(len(L)):
    ...
    cs=CubicSpline(C[minimums[i]-3:minimums[i]+3],A[minimums[i]-3:minimums[i]+3,i])
