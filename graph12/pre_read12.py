...
for ii in range(num_b):
    ...
    File=pd.read_csv("./graph12/"+BB[ii],delim_whitespace=True,header=None)
    A[:,ii]=File[0]
    B[:,ii]=File[1]
...
write_text(np.array([pca,err_pca]),"./graph12/DP_L"+str(L)+"T"+str(t)+"P("+str(pp)+"-"+str(pp+dp)+")S"+str(L)+"N"+str(N)+"Z"+str(zas)+".aux")
