...
AA=os.listdir("./graph9/")
BB=[]
L=[]
t=[]

for x in AA:
    if re.match(str(allsys)+"_L[0-9]+T[0-9]+P\("+str(pp)+"-"+str(pp+dp)+"\)DId.aux",x):
        BB.append(x)
print("files:",BB)
...
plt.title(r"$DIdS(p)$",fontsize=14)
plt.xlabel(r"$p$",fontsize=14)
plt.ylabel(r"$DId$",fontsize=14)


for ii in range(len(L)):
    A=pd.read_csv("./graph9/"+str(allsys)+"_L"+str(L[ii])+"T"+str(t[ii])+"P("+str(pp)+"-"+str(pp+dp)+")DId.aux",delim_whitespace=True,header=None)
    jj=np.where(A[1] == A[1].max())[0][0]
    Pc[ii]=A[0][jj]
    plt.scatter(A[0],A[1],label="L="+str(L[ii]))

plt.legend()    
plt.savefig("./graph9/"+str(allsys)+"_"+str(pp)+"P"+str(dp)+"DP"+str(N)+"N"+"DS.pdf")
...
plt.legend()
plt.savefig("./graph9/"+str(allsys)+"_"+str(pp)+"P"+str(dp)+"DP"+str(N)+"N"+"REGmin.pdf")
