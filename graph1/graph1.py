...
plt.ylim([0.1,1])
for i in range(3):
    B=pd.read_csv("./graph1/DP_OP_L"+sys.argv[1]+"T"+sys.argv[2]+"P"+sys.argv[3+i]+".txt",delim_whitespace=True,header=None)
    eps=B[0]
    zet=B[1]
    iot=B[2]
    ...
    if i==1:
        ...
        plt.plot(x,np.e**(Result[0])*x**(Result[2]),label=r"$\rho=A*t^{\delta}$"+"\n" +"$\delta="+str(round(Result[2],3))+"\pm"+str(round(Result[3],4))+"$")
plt.tight_layout()
plt.legend()
plt.savefig("./graph1/graph1.png")
